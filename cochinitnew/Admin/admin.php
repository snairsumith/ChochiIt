...
function logincheckSecadmin($name,$pass)
{
	$sql="select * from secadmdet where uname='$name' and pass='$pass'";
	$res= mysql_query($sql);
	return $res;
}
function AddCat($cat)
...
function insertProduct($pname,$pdesc,$pamt,$img)
{
	$sql="INSERT INTO `cochinit`.`productdetails` (`PrdtName`, `PrdtDate`, `PrdtDesc`, `PrdtAmnt`, `Image`) VALUES ('$pname',now() ,'$pdesc', '$pamt', '$img')";
	echo $sql;
	$res=mysql_query($sql);
	return $res;
}
function insertPrject($title,$desc,$mod,$tech,$bend,$amt,$abst,$psandpd,$uname)
{
	$sql="INSERT INTO `cochinit`.`projectdetails` (`PrjTitle`, `PrjModules`, `Prjtec`, `PrjBackend`, `PrjAmt`, `PrjAbstract`, `Username`, `Date`) VALUES ('$title', '$mod', 'tech', '$bend', '$amt', '$abst', '$uname', now());";
	
	$res=mysql_query($sql);
	return $res;
} 
function insertseminar($title,$sdesc,$stech,$amt,$fdoc,$unm)
{
	$sql="INSERT INTO `cochinit`.`seminardetails` (`semTitle`, `semDesc`, `tec`, `amount`, `date`, `username`, `files`) VALUES ('$title','$sdesc', '$stech', '$amt',now(),'$unm','$fdoc');";
	$res=mysql_query($sql);
	return $res;
}
...
function selectUserUname()
{
	$sql="select * from login where role='User'";
	$res=mysql_query($sql);
	return $res;
}
function selectEmpUname()
{
	$sql="select * from secadmdet";
	$res=mysql_query($sql);
	return $res;
}
